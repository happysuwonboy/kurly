...
if (localStorage.getItem('popup-banner-hide')) {
  let tomorrowMidnight =
    JSON.parse(localStorage.getItem('popup-banner-hide')) / 1000;
  let now = new Date().getTime() / 1000;
...
document
  .querySelector('.popup-banner button.close-popup')
  .addEventListener('click', (e) => {
    document.querySelector('.popup-banner').style.display = 'none';
  });
document
  .querySelector('.popup-banner button.close-popup-tomorrow')
  .addEventListener('click', (e) => {
    document.querySelector('.popup-banner').style.display = 'none';
    ...
    localStorage.setItem(
      'popup-banner-hide',
      JSON.stringify(tomorrowMidnight.getTime())
    );
  });
